...
class Expression {
 public:
  ...
  bool eval_game_object(Game_object **value);
	bool eval_animation_block(Animation_block **value);

  bool type(Gpl_type &type);
	bool is_array() { return (var_ != NULL && var_->is_array()) ? true : false; }

	Variable *get_variable() { return var_; }

  friend ostream &operator<<(ostream &os, Expression &exp);

 private:
	int eval_comparison();
  int eval_game_object_comp(Game_object *lobj, Game_object *robj);
