...
vector<Animation_block*> anim_vec;
...
%type <union_int>						object_type object_type_declaration check_animation_parameter
%type <union_type>    			simple_type 
%type <union_op_type> 			math_operator geometric_operator
...
program:
		declaration_list block_list
    {
      ...
      // check each animation_block for validity, they are only marked 
      // complete once we match the animation_block action
      for (it = anim_vec.begin(); it != anim_vec.end(); ++it) {
        Animation_block *anim_blk = *it;

        if (!anim_blk->complete()) {
          Error::error(Error::NO_BODY_PROVIDED_FOR_FORWARD, anim_blk->name());
        }
      }
    }
    ;
...
//---------------------------------------------------------------------
// Created new grammar rule to create global game_objects but leaves
// the object_type rule intact to be used else where
...
forward_declaration:
    T_FORWARD T_ANIMATION T_ID T_LPAREN animation_parameter T_RPAREN
      ...
      if (Symbol_table::instance()->Exists(*$3)) {
        Error::error(Error::PREVIOUSLY_DECLARED_VARIABLE, *$3);
      } else {
        Symbol_table::instance()->Put(sym);

        // push the new animation block onto out global list
        anim_vec.push_back(sym->get_animation_block());
      }
		}
    ;
...
initialization_block:
    ...
    {
      // adds initialize event 
      Event_manager::instance()->set_stmt(Window::INITIALIZE, $2);
    }
    ;

//---------------------------------------------------------------------
animation_block:
    animation_declaration T_LBRACE statement_list T_RBRACE end_of_statement_block
    ;

//---------------------------------------------------------------------
// Created rule to handle animation block declarations
animation_declaration:
    T_ANIMATION T_ID T_LPAREN check_animation_parameter T_RPAREN
    {
      // Get animation_block symbol, should have been created
      // during the forward declaration
      Symbol *symbol = Symbol_table::instance()->Get(*$2);

      // We shouldn't find a symbol if there was no forward declaration
      if (symbol == NULL) {
        Error::error(Error::NO_FORWARD_FOR_ANIMATION_BLOCK, *$2); 
      } else {
        Symbol *anim_sym = symbol->get_animation_block()->get_parameter_symbol();

        // Check if parameter has already been marked complete, if true
        // we've got a duplicate animation_block
        if (symbol->get_animation_block()->complete()) {
          Error::error(Error::PREVIOUSLY_DEFINED_ANIMATION_BLOCK, symbol->name());
        } else {
          bool type_error = false;
          string type_name = anim_sym->get_gobject_value()->type();

          // Match the forward declaration parameter type to the parameter type
          // of the animation parameter type
          switch ($4) {
          case T_TRIANGLE:
            if (type_name != "Triangle") type_error = true;
            break;
          case T_PIXMAP:
            if (type_name != "Pixmap") type_error = true;
            break;
          case T_CIRCLE:
            if (type_name != "Circle") type_error = true;
            break;
          case T_RECTANGLE:
            if (type_name != "Rectangle") type_error = true;
            break;
          case T_TEXTBOX: 
            if (type_name != "Textbox") type_error = true;
            break;
          default: assert(false);
          }

          // Throw error on mismatch type
          if (type_error) {
            Error::error(Error::ANIMATION_PARAM_DOES_NOT_MATCH_FORWARD);
          }

          // Push animation_block onto global stack to fill out the
          // statement block
          stmt_stack.push_back(symbol->get_animation_block());      

          // mark as complete for future validation and duplication
          // checking
          symbol->get_animation_block()->mark_complete();
        }
      }
    }
    ;
...
check_animation_parameter:
    T_TRIANGLE T_ID { $$ = T_TRIANGLE; }
    | T_PIXMAP T_ID { $$ = T_PIXMAP; }
    | T_CIRCLE T_ID { $$ = T_CIRCLE; }
    | T_RECTANGLE T_ID { $$ = T_RECTANGLE; }
    | T_TEXTBOX T_ID { $$ = T_TEXTBOX; }
    ;
...
expression:
    ...
    {
      Gpl_type rtype, ltype;

      $1->type(ltype);

      $3->type(rtype);
  
      // check for valid arguement type for geometric_opertors
      if (ltype != GAME_OBJECT) {
        Error::error(Error::OPERAND_MUST_BE_A_GAME_OBJECT, $1->id()); 
 
        $$ = new Expression(0);
      } else if (rtype != GAME_OBJECT) {
        Error::error(Error::OPERAND_MUST_BE_A_GAME_OBJECT, $3->id()); 

        $$ = new Expression(0);
      } else {
        $$ = create_binary_expression($2, new Expression($1), new Expression($3), GAME_OBJECT, GAME_OBJECT);
      }
    }
    ;
