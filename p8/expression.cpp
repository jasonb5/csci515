...
bool Expression::eval_game_object(Game_object **value) {
  Gpl_type t;

  var_->type(t);

  if (t != GAME_OBJECT) return false;

  if (var_->is_array()) {
    var_->eval_symbol();
  }

  *value = var_->get_game_object();
...
int Expression::eval_comparison() {
  ...
  case GAME_OBJECT: {
    Game_object *lobj, *robj;

    lexp_->eval_game_object(&lobj);

    rexp_->eval_game_object(&robj);

    return eval_game_object_comp(lobj, robj); 
  }
	default: assert(false);
	}
...
int Expression::eval_game_object_comp(Game_object *lobj, Game_object *robj) {
  switch (op_type_) {
  case TOUCHES: return lobj->touches(robj);
  case NEAR: return lobj->near(robj);
  default: assert(false);
  }
